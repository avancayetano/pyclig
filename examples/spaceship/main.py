...
class GameWindow(pyclig.window.Window):
	def __init__(self, *args, **kwargs):
		...
		self.score = pyclig.label.Label(window=self, text="SCORE: {}".format(self.player.score), x=0, y=self.height - 2)

		self.cursor = 0

	# overwritten on_press method of the Window class (see the pyclig.window.Window class)
	...
	def run(self, stdscr):
		...
		while self.running:
			score_text = "SCORE: {}".format(self.player.score)
			self.score.update(score_text)
			self.score.render()
			if self.player.state == "dead":
				self.title.render()
